	...
	function schedulerName(response) {
		...
		$(document).ready(function() {
			...
			if (response === 'Round Robin 1 Quantum NP') {
				...
				$('#textContent').text(
					"Iremos falar um pouco sobre a técnica de escalonamento Round Robin, que também se conhece por 'RR'. Basicamente, o escalonamento de Round Robin usa fatias do tempo iguais e em ordem circular, manipulando todos os processo, independente da prioridade."
				)
			}
			...
			case 'Round Robin 1 Quantum P':
				function animationRrOneP() {
					...
					let $box = $('#box'),
						...
						$textBox = $('#textBox'),
						$repeatAnimation = $('#repeatAnimation')
					;($boxes = $('.box')), ($textTime = $('#textTime')), (tl = new TimelineLite())
